...
void Event::current_room_event() {
	...
	// 주임원사실
		...
		Screen::print("[통신물자 창고]");
		Screen::print("통신관련 물품들이 여기저기 어질러져 있다..");
		Screen::print("누군가 도망가려 했던 흔적들이 보인다.");
	}
...
void Event::search_event() {
	...
	if (room_name == "2층 동편") {
		...
		Weapon* tmp = new Weapon("[부러진 대걸레]");
		player->get_bag()->insert_item(tmp);
	}
	else if (room_name == "세탁실") {
		Screen::line_print("[차갑게 식은 냉동 만두]를 발견했다.");
		Food* tmp = new Food("[차갑게 식은 냉동 만두]");
		//tmp->set_info(player);
		player->get_bag()->insert_item(tmp);
	}
	else if (room_name == "격리 생활관") {
		Screen::line_print("[칸막이 열쇠]를 발견했다.");
		Key* tmp = new Key("[칸막이 열쇠]");
		player->get_bag()->insert_item(tmp);
	}
	else if (room_name == "간부 연구실") {
		// 탐색 이벤트 X
	...
	// 2층 중앙 복도
		...
		Food* tmp = new Food("[전투 식량 파운트 케이크]");
		//tmp->set_info(player);
		player->get_bag()->insert_item(tmp);
	}
	else if (room_name == "1-2") {
		...
		Food* tmp = new Food("[야채 건빵 한 봉지]");
		//tmp->set_info(player);
		player->get_bag()->insert_item(tmp);
	}
	else if (room_name == "1-3") {
		// 몽키 스패너
		Screen::line_print("[몽키스패너]를 발견했다.");
		Weapon* tmp = new Weapon("[몽키스패너]");
		player->get_bag()->insert_item(tmp);
	}
	else if (room_name == "2-1") {
		// 성경책
		Screen::line_print("[두꺼운 성경책]를 발견했다.");
		Others* tmp = new Others("[두꺼운 성경책]");
		player->get_bag()->insert_item(tmp);
	}
	else if (room_name == "2-2") {
		// 붕대
		Screen::line_print("[붕대]를 발견했다.");
		Medecine* tmp = new Medecine("[붕대]");
	/*	tmp->set_info(player);*/
		player->get_bag()->insert_item(tmp);
	}
	else if (room_name == "2-3") {
		...
		Medecine* tmp = new Medecine("[손 소독제]");
		//tmp->set_info(player);
		player->get_bag()->insert_item(tmp);
	}
	...
	// 총기함실
		...
		if (player->get_bag()->get_check_item("[총기함 키]")) {
			...
			Weapon* tmp = new Weapon("[K2 소총]");
			player->get_bag()->insert_item(tmp);
		}
		else {
			Screen::line_print("총기함은 굳게 잠겨있다.");
			return;
		}
	}
	// 인사과
		...
		Key* tmp = new Key("[총기함 키]");
		player->get_bag()->insert_item(tmp);
		Medecine* tmp2 = new Medecine("[구급약 키트]");
		//tmp2->set_info(player);
		player->get_bag()->insert_item(tmp2);
	}
	// 지휘통제실
	else if (room_name == "지휘통제실") {
		Screen::line_print("좀비가 입고있던 전투복에서 [주임원사실 열쇠]를 발견했다.");
		Screen::line_print("주임원사실에 들어갈 수 있을 것이다.");
		Key* tmp = new Key("[주임원사실 열쇠]");
		player->get_bag()->insert_item(tmp);
	}
	// 군수과
	else if (room_name == "군수과") {
		Screen::line_print("[어딘가 중요해 보이는 열쇠]를 발견했다.");
		Key* tmp = new Key("[어딘가 중요해 보이는 열쇠]");
		player->get_bag()->insert_item(tmp);
	}
	// 대대장실
	else if (room_name == "대대장실") {
	...
	// 통신물자 창고
		...
		Others* tmp = new Others("[노트북과 연결 포트]");
		player->get_bag()->insert_item(tmp);
	}
...
void Event::partition_event() {
	if (this->player->get_bag()->get_check_item("[칸막이 열쇠]")) {
		Screen::print("철커덕!");
		Item* tmp = player->get_bag()->get_item("[칸막이 열쇠]");
...
void Event::mid_way_2() {
	...
	if (input == 0) {
		...
		if (this->player->get_bag()->get_check_item("[부러진 대걸레]")) {
			...
			Item* tmp = player->get_bag()->get_item("[부러진 대걸레]");
...
void Event::control_center_room() {
	...
	if (input == 0) {
		...
		if (
			...
			if (player->get_bag()->get_check_item("[몽키 스패너]")) {
				Item* tmp = player->get_bag()->get_item("[몽키 스패너]");
				player->get_bag()->delete_item("[몽키 스패너]");
				tmp->use_item();
			}
			else if (player->get_bag()->get_check_item("[K2 소총]")) {
				Item* tmp = player->get_bag()->get_item("[K2 소총]");
				player->get_bag()->delete_item("[K2 소총]");
				tmp->use_item();
			}
			else {
				Item* tmp = player->get_bag()->get_item("[권총과 탄창]");
				player->get_bag()->delete_item("[권총과 탄창]");
				tmp->use_item();
			}
			// 큰 부상을 당함(체력 감소)
...
void Event::sergeant_major() {
	...
	// 열쇠가 있는 경우
		...
		Item* tmp = player->get_bag()->get_item("[주임원사실 열쇠]");
		player->get_bag()->delete_item("[주임원사실 열쇠]");
		tmp->use_item();
		...
		if (input == 0) {
			...
			if (player->get_bag()->get_check_item("[어딘가 중요해 보이는 열쇠]")) {
				...
				Screen::print("...........");
				Screen::print("드Z 철컥!");
				Item* tmp = player->get_bag()->get_item("[어딘가 중요해 보이는 열쇠]");
				player->get_bag()->delete_item("[어딘가 중요해 보이는 열쇠]");
				tmp->use_item();
				Screen::print("중요해보이는 기밀 문서들이 잔뜩 있다.");
				Screen::line_print("[권총과 탄알집]을 발견했다.");
				Weapon* tmp2 = new Weapon("[권총과 탄알집]");
				player->get_bag()->insert_item(tmp2);
...
void Event::end_event() {
	...
	if (player->get_bag()->get_check_item("[옥상 열쇠]")) {
		Item* tmp = player->get_bag()->get_item("[옥상 열쇠]");
		player->get_bag()->delete_item("[옥상 열쇠]");
		tmp->use_item();
		while (1) {
			Screen::text_clear();
			Screen::print("[옥상]");
			...
			if (input == 0) {
				...
				if (player->get_bag()->get_check_item("[노트북과 연결 포트]")) {
					// 통신망 개통
					Item* tmp = player->get_bag()->get_item("[노트북과 연결 포트]");
					player->get_bag()->delete_item("[노트북과 연결 포트]");
					tmp->use_item();
